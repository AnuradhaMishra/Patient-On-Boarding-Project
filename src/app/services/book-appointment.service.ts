...
export class BookAppointmentService {
  ...
  validateAppointment(appointmentNumber:number, patientId:number, patientName:string, patientAge:number, reasonToVisit:string, dateAndtime:string, consultationFee:number): Observable<BookAppointment>{
    return this.http.get<BookAppointment>(`${appointmentUrl}`)
    .pipe(
      retry(1),
      catchError(this.errorHandler)
